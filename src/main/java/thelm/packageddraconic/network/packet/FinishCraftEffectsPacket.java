			...
			if(level.isLoaded(pos)) {
				if(doParticles) {
					...
					for(int i = 0; i < 100; i++) {
						...
						level.addParticle(new IntParticleType.IntParticleData(DEParticles.energy_basic, 0, 255, 255, 64), pos.getX()+0.5, pos.getY()+0.5, pos.getZ()+0.5, velX, velY, velZ);
					}
				}
				level.playLocalSound(pos.getX()+0.5, pos.getY()+0.5, pos.getZ()+0.5, DESounds.fusionComplete, SoundSource.BLOCKS, 4F, (1F+(level.random.nextFloat()-level.random.nextFloat())*0.2F)*0.7F, false);
			}
		});
